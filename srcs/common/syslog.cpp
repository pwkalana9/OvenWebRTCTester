...
CSysLog::CSysLog(TCHAR *ptcIdent)
{
...
bool CSysLog::Open(TCHAR *ptcFileName)
{
    if (ptcFileName)
    {
...
void CSysLog::SysLog(TCHAR *ptcFormat, ...)
{
    va_list vl;
...
void CSysLog::WriteLogV(TCHAR *ptcFormat, va_list vl)
{
	LOCK_LOG();
...
void CSysLog::SetDailyBackupTime(int iHour)
{
    if (iHour < 0 || iHour > 23)
    {
        m_iLastBackupDay = -1;
...
WINDOWS_TYPE CSysLog::GetWindowsType()
{
...
int CSysLog::GetWriteBufferSize(const TCHAR tcDrive, const WINDOWS_TYPE eWinType)
{
