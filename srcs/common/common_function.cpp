...
bool LogInit(const char * file_name, int nBackupHour) 
{
	...
	bool result = 	gSysLog.Open((char *)file_name);
	
	gSysLog.SetDailyBackupTime(nBackupHour); 
...
void LogPrint(const char * pFormat, ...)
{
	...
	char 			szLog[MAX_LOG_TEXT_SIZE + 100] = {0, };
	std::string		time_string; 
	struct tm *		pCurrnetTime;
	time_t			timer;
	
	timer = time(nullptr);
	pCurrnetTime = localtime(&timer); // 초 단위의 시간을 분리하여 구조체에 넣기
	
	
	va_list argument;
	va_start(argument,pFormat);

	vsprintf(szLog, pFormat, argument);

	va_end(argument);

	//화면 출력 
	printf("\r\n[%04d-%02d-%02d %02d:%02d:%02d]\t%s",	pCurrnetTime->tm_year + 1900, 
														pCurrnetTime->tm_mon + 1, 
														pCurrnetTime->tm_mday, 
														pCurrnetTime->tm_hour, 
														pCurrnetTime->tm_min, 
														pCurrnetTime->tm_sec, 
														szLog);
	fflush(stdout);
