...
bool ThreadTimer::Create(ITimerCallback *callback, uint32_t Resolution)
{
	...
	m_bStopThread = false;
#ifdef WIN32
	m_hThread = (HANDLE)_beginthreadex(nullptr, 0, _TimerThread, (void*)this, 0, nullptr);
#else
	pthread_create(&m_hThread, nullptr, _TimerThread, this);
#endif
	if( !m_hThread ) { goto $error_occurred; }
...
bool ThreadTimer::SetTimer(uint32_t timer_id, uint32_t Elapse) // 단위는 ms
{
	...
	else
	{
		...
		// 추가
		_timer_map.insert(std::pair<uint32_t, PTIMER_INFO>(timer_id, pt_timer));
	}
...
uint32_t ThreadTimer::_TimerThreadMain( )
{
	while(!m_bStopThread)
	{
		// 쉬기
...
#ifdef WIN32
uint32_t __stdcall ThreadTimer::_TimerThread(LPVOID pParameter)
{
	return ((ThreadTimer*)pParameter)->_TimerThreadMain();
}
#else
void* ThreadTimer::_TimerThread(void* pParameter)
{

	((ThreadTimer*)pParameter)->_TimerThreadMain();
