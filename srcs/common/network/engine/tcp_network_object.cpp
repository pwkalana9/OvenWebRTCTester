...
TcpNetworkObject::~TcpNetworkObject()
{
	//LOG_WRITE(("[%s] - TcpNetworkObject::~TcpNetworkObject", _object_name.c_str()));
...
bool TcpNetworkObject::Create(TcpNetworkObjectParam *param)
{
	...
	if(param->socket == nullptr && param->enable_ssl == true)
	{
		...
		try
		{
			...
			_io_service			= &(_socket_ssl->lowest_layer().get_io_service());//param->pIoService;
		}
	...
	else
	{
		...
		try
		{
			...
			_io_service			= &(_socket->get_io_service());//param->pIoService;
		}
...
// KeepAlive
//====================================================================================================
void TcpNetworkObject::SetKeepAlive()
{
	int 	socket = -1;
	...
	if(_is_support_ssl == true)
	{
		socket = _socket_ssl->lowest_layer().native_handle();
		_socket_ssl->lowest_layer().set_option(option);
		
	}
	else
	{
		socket = _socket->native_handle();
	...
	nReturn = setsockopt(socket, SOL_SOCKET, SO_KEEPALIVE, (char*)&bOptVal, bOptLen);

	tcp_keepalive  keepalive_setting;
	keepalive_setting.onoff				= 1;		// no MS documentation, so I assume it's a bool
	keepalive_setting.keepalivetime		= 50000;	// this value is in milliseconds
	keepalive_setting.keepaliveinterval	= 50000;	// this value is in milliseconds

	nReturn = WSAIoctl(socket, SIO_KEEPALIVE_VALS, &keepalive_setting, sizeof(tcp_keepalive), 0,0,&dwRetval,0,0);
	...
	setsockopt(socket, SOL_TCP, 	TCP_KEEPIDLE, 	&idle, 	sizeof(idle));
	setsockopt(socket, SOL_TCP, 	TCP_KEEPINTVL, 	&intvl, sizeof(intvl));
	setsockopt(socket, SOL_TCP, 	TCP_KEEPCNT, 	&cnt, 	sizeof(cnt));
#endif
}
...
void TcpNetworkObject::PostClose()
{
	...
	_post_close_timer = new NetTimer(*_io_service);
...
bool TcpNetworkObject::PostSend(std::shared_ptr<std::vector<uint8_t>> data, bool is_data_copy/*=false*/) 
{
	...
	if(_max_send_data_size > 0 && (_max_send_data_size < (int)data->size()))
	{
...
//
	...
	//처리 에러 
	if(process_size < 0 || process_size > (int)_recv_data->size())
	{
	...
	//남은 데이터 크기 
	int rest_data_size = _recv_data->size() - process_size;
...
void TcpNetworkObject::SetKeepAliveSendTimer(int interval, TcpKeepaliveSendCallback callback)
{
	...
	_keep_alive_send_timer	 	= new NetTimer(*_io_service);
	_keepalive_send_callback 	= callback; 
	SetNetworkTimer(_keep_alive_send_timer, (int)NetworkTimerId::KeepaliveSend, interval);
...
void TcpNetworkObject::SetKeepAliveCheckTimer(int interval, TcpKeepAliveCheckCallback callback)
{
	...
	_keepalive_check_timer	 	= new NetTimer(*_io_service);
	_keepalive_check_callbak 	= callback; 
	SetNetworkTimer(_keepalive_check_timer, (int)NetworkTimerId::KeepaliveCheck, interval);
}
...
void TcpNetworkObject::SetCloseTimer()
{
	...
	_close_timer = new NetTimer(*_io_service);
	SetNetworkTimer(_close_timer, (int)NetworkTimerId::Close, DEFAULT_NETWORK_CLOSE_TIMER_INTERVAL);
}
...
{	
	...
	if(_is_closeing == true)
	{
		if(id == (int)NetworkTimerId::PostClose)
		{
	...
	switch(id)
	{
	case (int)NetworkTimerId::KeepaliveSend :
		{
			// 함수 포인터 호출	
		...
		case (int)NetworkTimerId::KeepaliveCheck:
		{
