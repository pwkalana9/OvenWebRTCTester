...
void TcpNetworkManager::PostRelease()
{
	...
	if(_acceptor != nullptr)
	{
		...
		_release_timer = new NetTimer(_acceptor->get_io_service());
...
bool TcpNetworkManager::Create(INetworkCallback * callback, 
								std::shared_ptr<NetworkServicePool> service_pool, 
								int listen_port, 
...
{
	auto * socket = new NetTcpSocket(*(_service_pool->GetService())); 
...
bool TcpNetworkManager::PostConnectSSL(uint32_t ip, std::string host, int port, std::shared_ptr < std::vector<uint8_t>> connected_param)
{
	...
	auto * socket = new NetSocketSSL(*(_service_pool->GetService()), *_ssl_context);
...
{
	NetworkConnectedResult result= NetworkConnectedResult::Success;
	...
	if(error)
	{
		...
		result = NetworkConnectedResult::Fail;
		
	}
	else 
	{
		result = NetworkConnectedResult::Success;
...
bool TcpNetworkManager::PostAccept()
{
	if(_acceptor == nullptr)
	{
		try
		{
			...
			_acceptor = new NetAcceptor(_is_private_accepter_service == true ? *(_service_pool->GetPrivateService()) : *(_service_pool->GetService()));
	...
	if(_is_support_ssl == true)
	{
		...
		_accept_socket_ssl = new NetSocketSSL(*(_service_pool->GetService()), *_ssl_context);
	...
	else
	{
		...
		_accept_socket  = new NetTcpSocket(*(_service_pool->GetService())); 
