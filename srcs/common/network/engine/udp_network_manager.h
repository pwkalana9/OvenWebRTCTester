...
public : 
	virtual bool		Create(INetworkCallback * callback, std::shared_ptr<NetworkServicePool> service_pool, int listen_port, char * object_name = nullptr);
	virtual bool		Create(INetworkCallback * callback, std::shared_ptr<NetworkServicePool> service_pool, char * object_name = nullptr) { return Create(callback, service_pool, 0, object_name); }
...
protected : 	
	std::shared_ptr<NetworkServicePool> _service_pool;
	int					_index_key; 
