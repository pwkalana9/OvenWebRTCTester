...
struct TlsCallback
{
	...
	std::function<size_t(Tls *tls, void *buffer, size_t length)> read_callback = nullptr;

	// Return value: written bytes
	std::function<size_t(Tls *tls, const void *data, size_t length)> write_callback = nullptr;
...
public:
	...
	bool Initialize(const SSL_METHOD *method, const std::shared_ptr<Certificate> &certificate, const std::shared_ptr<Certificate> &chain_certificate, const std::string &cipher_list, TlsCallback callback);
	...
	std::shared_ptr<std::vector<uint8_t>> Read();
	...
	bool ExportKeyingMaterial(unsigned long crypto_suite, const std::string &label, std::shared_ptr<std::vector<uint8_t>> &server_key, std::shared_ptr<std::vector<uint8_t>> &client_key);
...
protected:
	...
	bool PrepareSslContext(const SSL_METHOD *method, const std::shared_ptr<Certificate> &certificate, const std::shared_ptr<Certificate> &chain_certificate, const std::string &cipher_list);
	bool PrepareBio();
