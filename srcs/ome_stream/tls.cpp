...
BIO_METHOD *Tls::PrepareBioMethod()
{
	...
	{
		bio_methods = ::BIO_meth_new(BIO_TYPE_MEM, "Tls");
...
bool Tls::Initialize(const SSL_METHOD *method, const std::shared_ptr<Certificate> &certificate, const std::shared_ptr<Certificate> &chain_certificate, const std::string &cipher_list, TlsCallback callback)
{
	bool result = true;
...
bool Tls::PrepareSslContext(const SSL_METHOD *method, const std::shared_ptr<Certificate> &certificate, const std::shared_ptr<Certificate> &chain_certificate, const std::string &cipher_list)
{
	do
	{
		...
		if(ctx == nullptr)
		{
			LOG_WRITE(("ERROR : Cannot create SSL context"));
			break;
		}
		...
		{
			LOG_WRITE(("ERROR : Cannot use certficate"));
			break;
		}
		...
		{
			LOG_WRITE(("ERROR : Cannot use chain certificate"));
			break;
		}

		if(::SSL_CTX_use_PrivateKey(ctx, certificate->GetPkey()) != 1)
		{
			LOG_WRITE(("ERROR : Cannot use private key"));
			break;
		}
		...
		int result = ::SSL_CTX_set_cipher_list(ctx, cipher_list.c_str());
		if (result != 1)
		{
			LOG_WRITE(("ERROR : SSL_CTX_set_cipher_list Fail"));
			break;
		}
		...
		result = DO_CALLBACK_IF_AVAILBLE(bool, false, this, create_callback, static_cast<SSL_CTX *>(_ssl_ctx));

		if(result == false)
		{
			LOG_WRITE(("ERROR : An error occurred inside create callback"));
			_ssl_ctx = nullptr;
...
int Tls::TlsRead(BIO *b, char *out, int outl)
{
	...
	auto read_bytes = DO_CALLBACK_IF_AVAILBLE(size_t, -1, BIO_get_data(b), read_callback, out, static_cast<size_t>(outl));
...
int Tls::TlsWrite(BIO *b, const char *in, int inl)
{
	...
	auto written_bytes = DO_CALLBACK_IF_AVAILBLE(size_t, -1, BIO_get_data(b), write_callback, in, static_cast<size_t>(inl));
...
int Tls::Connect()
{
	...
	default:
		// Another error
		LOG_WRITE(("ERROR : An error occurred while connect SSL connection: code: %d, error: %d (errno: %d)", result, error, errno));
		// OV_ASSERT2(false);
		break;
	}
...
std::shared_ptr<std::vector<uint8_t>> Tls::Read()
{
...
std::shared_ptr<Certificate> Tls::GetPeerCertificate()
{
	...
	if(cert == nullptr)
	{
		LOG_WRITE(("ERROR : An error occurred while get peer certificate"));
		return nullptr;
	}
...
bool Tls::ExportKeyingMaterial(unsigned long crypto_suite, const std::string &label, std::shared_ptr<std::vector<uint8_t>> &server_key, std::shared_ptr<std::vector<uint8_t>> &client_key)
{
	OV_ASSERT2(_ssl != nullptr);
	...
	auto key_data = std::make_shared<std::vector<uint8_t>>(capacity);
	key_data->resize(capacity);
		
	auto key_buffer = key_data->data();

	int result = ::SSL_export_keying_material(_ssl, key_buffer, key_data->size(), label.c_str(), label.size(), nullptr, 0, false);
	...
	client_key->insert(client_key->end(), key_buffer, key_buffer + key_len);
	key_buffer += key_len;

	server_key->insert(server_key->end(), key_buffer, key_buffer + key_len);
	key_buffer += key_len;

	client_key->insert(client_key->end(), key_buffer, key_buffer + salt_len);
	key_buffer += salt_len;

	server_key->insert(server_key->end(), key_buffer, key_buffer + salt_len);
